...
class ConcatClassTokenAddPosEmbed(nn.Module):
    def __init__(self, embed_dim=768, num_patches=49):
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, qkv_bias=False, qk_scale=None, drop_ratio=0., attn_drop_ratio=0., drop_path_ratio=0.):
        ...
        self.mlp = MLP(in_features=dim, drop=drop_ratio)
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, embed_dim=768,
        ...
        dpr = torch.linspace(0., drop_path_ratio, depth).tolist()  # stochastic depth decay rule
        ...
        # Handle representation size
        if representation_size:
            self.has_logits = True
            self.pre_logits = nn.Linear(embed_dim, representation_size)
        else:
            self.has_logits = False
            self.pre_logits = nn.Identity()  # Equivalent to no-op linear activation
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        x = self.pre_logits(x[:, 0])  # Apply pre_logits only to class token
        x = self.head(x)
...
def vit_base_patch16_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    ...
    pretrained_weights = torch.load("ViT-B_16.pth") #path of ViT-B_16.pth
...
def vit_base_patch32_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    ...
    model = VisionTransformer(img_size=224,
                              ...
                              representation_size=768 if has_logits else None,
                              num_classes=num_classes)
    pretrained_weights = torch.load("ViT-B_32.pth") #path of ViT-B_32.pth
    model.load_state_dict(pretrained_weights, strict=False)
    return model
...
def vit_large_patch16_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    ...
    model = VisionTransformer(img_size=224,
                              ...
                              num_classes=num_classes)
    pretrained_weights = torch.load("ViT-L_16.pth")  #path of ViT-L_16.pth
    model.load_state_dict(pretrained_weights, strict=False)
    return model
...
def vit_large_patch32_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    ...
    model = VisionTransformer(img_size=224,
                              ...
                              representation_size=1024 if has_logits else None,
                              num_classes=num_classes)
    pretrained_weights = torch.load("ViT-L_32.pth") #path of ViT-L_32.pth
    model.load_state_dict(pretrained_weights, strict=False)
    return model
