...
def compute_metrics(model_outputs, labels):
    ...
    real_cnt = (bin_labels == 0).sum()+ 1e-12
    fake_cnt = (bin_labels == 1).sum()+ 1e-12

    acc = (bin_preds == bin_labels).float().mean()

    real_acc = (bin_preds == bin_labels)[torch.where(bin_labels == 0)].sum() / real_cnt
    fake_acc = (bin_preds == bin_labels)[torch.where(bin_labels == 1)].sum() / fake_cnt
